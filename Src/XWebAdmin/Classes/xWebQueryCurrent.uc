...
//if _RO_
var localized string CurrentLinks[7];
//else
...
function bool Query(WebRequest Request, WebResponse Response)
{
    ...
    //if _RO_
	case CurrentResetPage:
		if (!MapIsChanging())
			QueryResetPage(Request, Response);
		return true;
    //end _RO_
	}
	return false;
...
function QueryCurrentMenu(WebRequest Request, WebResponse Response)
{
    ...
    //if _RO_
	case CurrentResetPage:
		Response.Subst("ResetBG",   HighlightedBG);
		break;
    //end _RO_
	}
    ...
    //if _RO_
	Response.Subst("ResetURI", 	    DefaultPage$"?Page="$CurrentResetPage);
	Response.Subst("ResetLink",	    CurrentLinks[6]);
    //end _RO_
...
defaultproperties
{
     ...
     //if _RO_
     CurrentResetPage="current_reset"
     CurrentLinks(6)="Reset Game"
     //end _RO_
