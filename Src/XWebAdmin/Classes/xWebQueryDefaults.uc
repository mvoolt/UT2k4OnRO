...
{
    local String GameType, ListName, Tmp, MapName, MapURL, NewMapURL, RoundLimitString, WinLimitString;
    ...
    local int i, Count, MoveCount, id, newId, CurrentList, Index;
	...
	if (CanPerform("Ml"))
	{
		...
		if (Tmp != "")
		{
			...
			switch (Tmp)
			{
			    ...
			    //if _RO_
				case "Update":
				    //Make sure the user actually has maps selected
					if (Request.GetVariableCount("IncludeMapsSelect") > 0)
					{
					    //Grab the new values for RoundLimit and WinLimit
    					RoundLimitString = Request.GetVariable("RoundLimit");
    					WinLimitString = Request.GetVariable("WinLimit");

    					//Loop through all selected maps
    					Count = Request.GetVariableCount("IncludeMapsSelect");
    					for (i = 0; i < Count; i++)
    					{
                            //Start the Map URL with the Map name
                            MapURL = Request.GetVariableNumber("IncludeMapsSelect", i);
                            MapName = class'MaplistRecord'.static.GetBaseMapName(MapURL);
                            NewMapURL = MapName;

    						//Add the new RoundLimit to the Map URL(if user put it in)
    						if (RoundLimitString == "-")
    						{
    						    //Grab the RoundLimit setting that's in the FullURL, if any
    						    NewMapURL $= Mid(MapURL, InStr(MapURL, "?RoundLimit="), 13);
    						}
    					    else if (RoundLimitString != "" && RoundLimitString != "0")
    					    {
    					        //Attach the new RoundLimit setting to the URL
                                NewMapURL $= "?RoundLimit=" $ RoundLimitString;
                            }

    						//Add the new WinLimit to the Map URL(if user put it in)
    						if (WinLimitString == "-")
    						{
    						    //Grab the RoundLimit setting that's in the FullURL, if any
    						    NewMapURL $= Mid(MapURL, InStr(MapURL, "?WinLimit="), 11);
    						}
    					    else if (WinLimitString != "" && WinLimitString != "0")
    					    {
    					        //Attach the new RoundLimit setting to the URL
                                NewMapURL $= "?WinLimit=" $ WinLimitString;
                            }

                            //Grab the map's index so we can move it back into place
                            id = IncludeMaps.FindTagId( MapName );

    					    //Set its new Full URL in the MapListManager
    					    MapListManager(Level.Game.MaplistHandler).SetMapURL( Index, CurrentList, id, NewMapURL );

    					    //Due to "SetAt" being protected, we must remove the old item, add a new one, and shift it back into its old position
    					    IncludeMaps.Remove(id);
    					    newId = IncludeMaps.Add(NewMapURL, MapName, true);
    					    id = id - newId;
    					    IncludeMaps.ShiftStrict(newId, id);

    					    //Copy the map into "MovedMaps" so that they get reselected on refresh
    						MovedMaps.CopyFrom(IncludeMaps, MapName);

    						//Force the Map List to save(more convenient/consistant for the admins)
    						bForceSave = true;
    					}
       				}
