...
class ROFastAutoFire extends ROAutomaticFire;
...
var ROHighROFWeaponAttachment	HiROFWeaponAttachment;      // A high ROF WA that this class will use to pack fire info
...
function HandleProjectileSpawning(vector SpawnPoint, rotator SpawnAim)
{
	...
	{
	   super(ROProjectileFire).SpawnProjectile(SpawnPoint, SpawnAim);
	   return;
	}

	if( HiROFWeaponAttachment == none )
	{
		HiROFWeaponAttachment = ROHighROFWeaponAttachment(Weapon.ThirdPersonActor);
	}
...
state FireLoop
{
    ...
    function ModeTick(float dt)
    {
		...
		if ( !bIsFiring || ROWeapon(Weapon).IsBusy() || !AllowFire() || (ROMGBase(Weapon) != none && ROMGBase(Weapon).bBarrelFailed) )  // stopped firing, magazine empty or barrel overheat
        {
			GotoState('');
			return;
		}
