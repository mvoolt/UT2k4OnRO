...
//
// Base class for German tank cannon pawns with rotational scope ranges
//===================================================================

class GermanTankCannonPawn extends ROTankCannonPawn
       abstract;
	...
	else if (!PC.bBehindView)
	{
        ...
        if ( DriverPositions[DriverPositionIndex].bDrawOverlays && !IsInState('ViewTransition'))
        {
			if( DriverPositionIndex == 0 )
			{
	    		...
	    		//log("Default rotation = "$default.ScopeCenterRotator.Rotation.Yaw$" or "$ScopeCenterRotator.default.Rotation.Yaw);

                Canvas.SetPos(posx + (Canvas.SizeY * (1.0 - ScopeCenterScale) / 2.0) ,
                    Canvas.SizeY * (1.0 - ScopeCenterScale) / 2.0);
				RotationFactor = 0;
				if( Gun != none )
				{
                	if( Gun.CurrentRangeIndex < 10 )
					{
						RotationFactor += ( Gun.CurrentRangeIndex * CenterRotationFactor );
					}
					else
					{
						RotationFactor += (	CenterRotationFactor * 10 ) + ((( Gun.CurrentRangeIndex - 10 ) * 2 ) * CenterRotationFactor );
					}
				}
