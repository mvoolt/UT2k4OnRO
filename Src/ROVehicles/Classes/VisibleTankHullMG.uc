...
class VisibleTankHullMG extends ROMountedTankMG
      abstract;
...
function HandleReload()
{
	if( NumMags > 0 && !bReloading)
	{
		...
		//log("Reloading duration = "$VehicleWeaponPawn(Owner).HUDOverlay.GetAnimDuration('reload_tankmg', 1.0));
...
simulated function ClientDoReload()
{
    if(Owner != none && VehicleWeaponPawn(Owner) != none && VehicleWeaponPawn(Owner).HUDOverlay != none )
		VehicleWeaponPawn(Owner).HUDOverlay.PlayAnim('reload_tankmg');
}
